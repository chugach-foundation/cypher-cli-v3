...
pub trait FuturesSubCommands {
    fn futures_subcommands(self) -> Self;
}

impl FuturesSubCommands for App<'_, '_> {
    fn futures_subcommands(self) -> Self {
        self.subcommand(
            SubCommand::with_name("futures")
                .about("Subcommands that interact with Futures Markets.")
                .subcommand(
...
pub fn parse_futures_command(matches: &ArgMatches) -> Result<CliCommand, Box<dyn error::Error>> {
    match matches.subcommand() {
        ("orders", Some(_matches)) => Ok(CliCommand::Futures(FuturesSubCommand::Orders)),
        ("cancel", Some(matches)) => {
            // market symbol
            ...
            let order_id = match matches.value_of("order-id") {
                ...
                None => {
                    return Err(Box::new(CliError::BadParameters(
                        "Order ID not provided, to see open orders try \"futures orders\" command."
                            .to_string(),
                    )));
                }
            };
            ...
            Ok(CliCommand::Futures(FuturesSubCommand::Cancel {
        ...
        ("close", Some(matches)) => {
            ...
            Ok(CliCommand::Futures(FuturesSubCommand::Close {
                symbol: symbol.to_string(),
            }))
        }
        ("settle", Some(matches)) => {
        ...
        ("market", Some(matches)) => {
            ...
            Ok(CliCommand::Futures(FuturesSubCommand::Market {
                symbol: symbol.to_string(),
                side,
                size,
            }))
        ...
        ("", None) => {
            ...
            Err(Box::new(CliError::CommandNotRecognized(
                "No futures subcommand given.".to_string(),
            )))
        }
        _ => unreachable!(),
    }
...
pub async fn list_futures_open_orders(
    config: &CliConfig,
...
pub async fn process_futures_cancel_order(
    config: &CliConfig,
    symbol: &str,
    order_id: u128,
    side: Side,
    ...
    println!(
        "Cancelling order with ID {} on Futures Market {}",
        order_id, market_name
    );

    let ixs = vec![cancel_futures_order(
        &public_clearing,
        &cache_account::id(),
        &master_account,
        &sub_account,
...
pub async fn process_futures_market_order(
    config: &CliConfig,
    symbol: &str,
    side: Side,
    size: I80F48,
    ...
    if impact_price.is_none() {
        return Err(Box::new(CliError::BadParameters(
            format!(
                "There is no available liquidity on the Order Book for Futures Market: {} to match size: {}",
                symbol,
                size
            )
        )));
...
pub async fn process_futures_close(
    config: &CliConfig,
    symbol: &str,
) -> Result<CliResult, Box<dyn error::Error>> {
    let rpc_client = config.rpc_client.as_ref().unwrap();
    ...
    let sub_account_state = match user_ctx.get_sub_account_with_position(&market.address) {
        ...
        None => {
            return Err(Box::new(CliError::BadParameters(format!(
                "Failed to get Sub Account with position for Futures Market: {}",
                symbol
            ))))
        }
    };
    ...
    let position = match sub_account_state.get_derivative_position(&market.address) {
        ...
        None => {
            return Err(Box::new(CliError::BadParameters(format!(
                "Failed to get Position for Futures Market: {}",
                symbol
            ))))
        }
    };
...
pub async fn process_futures_limit_order(
    config: &CliConfig,
    symbol: &str,
    side: Side,
    size: I80F48,
...
pub async fn process_futures_settle_funds(
    config: &CliConfig,
    symbol: &str,
) -> Result<CliResult, Box<dyn error::Error>> {
    let rpc_client = config.rpc_client.as_ref().unwrap();
    ...
    let ixs = vec![settle_futures_funds(
        &public_clearing,
        &cache_account::id(),
        &master_account,
        &sub_account,
